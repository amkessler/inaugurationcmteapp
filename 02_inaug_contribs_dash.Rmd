...
contribs_inaug_only <- contribs %>% 
  ...
  mutate(
    ...
    contributor_name = str_to_upper(contributor_name)
  ) %>% 
...
datatable(contribs_inaug_only,
      ...
      formatStyle(c('contributor_name', 'total_amount'), fontWeight = 'bold') 
...
### Total dollars from individuals

```{r}
sum_inddonors <- contribs %>% 
  filter(individual_or_organization == "Individual") %>% 
  summarise(total = sum(inaug_2017_total))


valueBox(dollar(sum_inddonors$total), icon = "fa-money")
...
```{r}
num_inddonors <- contribs %>% 
  filter(individual_or_organization == "Individual") %>% 
  nrow()


valueBox(num_inddonors, icon = "fa-user")

```


### Total dollars from organizations

```{r}
sum_orgdonors <- contribs %>% 
  filter(individual_or_organization == "Organization") %>% 
  summarise(total = sum(inaug_2017_total))


valueBox(dollar(sum_orgdonors$total), icon = "fa-money")
...
```{r}
num_orgdonors <- contribs %>% 
  filter(individual_or_organization == "Organization") %>% 
  nrow()


valueBox(num_orgdonors, icon = "fa-building")
...
Other Contributions
=====================================
This table shows donors to the Trump inauguration who have **also donated to other campaigns** in the past, according to [data](https://docs.google.com/spreadsheets/d/1qCG3BxuX42SNMW8u8tQ9HxkkZF-mFffnzTK6PaFxHGQ/edit?usp=sharing) compiled by the [Center for Responsive Politics](https://www.opensecrets.org).
...
datatable(contribs_all,
      ...
      formatCurrency(c("inaug_2017_total", "trump_campaign_2016", "other_2016_presidential_campaigns",
